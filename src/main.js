...
import district4GeoJsonRaw from './data/district-4.geojson?raw'
import district5GeoJsonRaw from './data/district-5.geojson?raw'
...
import district11GeoJsonRaw from './data/district-11.geojson?raw'
import district12GeoJsonRaw from './data/district-12.geojson?raw'
import district13GeoJsonRaw from './data/district-13.geojson?raw'
...
import district16GeoJsonRaw from './data/district-16.geojson?raw'
...
const district4GeoJson = JSON.parse(district4GeoJsonRaw)
const district5GeoJson = JSON.parse(district5GeoJsonRaw)
...
const district11GeoJson = JSON.parse(district11GeoJsonRaw)
const district12GeoJson = JSON.parse(district12GeoJsonRaw)
const district13GeoJson = JSON.parse(district13GeoJsonRaw)
const district14GeoJson = JSON.parse(district14GeoJsonRaw)
const district15GeoJson = JSON.parse(district15GeoJsonRaw)
const district16GeoJson = JSON.parse(district16GeoJsonRaw)
            ...
            <aside class="boundary-tool" aria-labelledby="boundary-tool-heading">
              <div class="boundary-tool__heading">
                <div>
                  ...
                  <h3 id="boundary-tool-heading">13지구 경계 입력</h3>
                </div>
...
function setupBoundaryTool(kakao) {
  ...
  boundaryTool.startButton.addEventListener('click', () => {
    ...
    setBoundaryGuide('지도에서 13지구 경계의 꼭짓점을 차례대로 선택하세요.')
  })
  ...
  boundaryTool.resetButton.addEventListener('click', () => {
    ...
    boundaryTool.startButton.textContent = '13지구 경계 그리기'
    updateBoundaryPreview()
  ...
  boundaryTool.completeButton.addEventListener('click', () => {
    ...
    setBoundaryGuide('13지구 경계를 완성했습니다. 아래에서 GeoJSON 좌표를 확인하세요.')
  })
...
function displayDistrict4Boundary(kakao) {
  const districtBoundary = district4GeoJson.geometry.coordinates[0]
  const boundaryPath = districtBoundary.map(
    ([longitude, latitude]) => new kakao.maps.LatLng(latitude, longitude),
  )
  ...
  const label = document.createElement('div')
  label.className = 'district-map-label'
  label.textContent = district4GeoJson.properties.name
...
function displayDistrict5Boundary(kakao) {
  const districtBoundary = district5GeoJson.geometry.coordinates[0]
  const boundaryPath = districtBoundary.map(
    ([longitude, latitude]) => new kakao.maps.LatLng(latitude, longitude),
  )
  ...
  const label = document.createElement('div')
  label.className = 'district-map-label'
  label.textContent = district5GeoJson.properties.name
...
function displayDistrict7Boundary(kakao) {
  const districtBoundary = district7GeoJson.geometry.coordinates[0]
  const boundaryPath = districtBoundary.map(
    ([longitude, latitude]) => new kakao.maps.LatLng(latitude, longitude),
  )
...
function displayDistrict9Boundary(kakao) {
  const districtBoundary = district9GeoJson.geometry.coordinates[0]
  const boundaryPath = districtBoundary.map(
    ([longitude, latitude]) => new kakao.maps.LatLng(latitude, longitude),
  )
  ...
  const label = document.createElement('div')
  label.className = 'district-map-label'
  label.textContent = district9GeoJson.properties.name
...
function displayDistrict10Boundary(kakao) {
  const districtBoundary = district10GeoJson.geometry.coordinates[0]
  const boundaryPath = districtBoundary.map(
    ([longitude, latitude]) => new kakao.maps.LatLng(latitude, longitude),
  )
  ...
  const label = document.createElement('div')
  label.className = 'district-map-label'
  label.textContent = district10GeoJson.properties.name
...
function displayDistrict11Boundary(kakao) {
  const districtBoundary = district11GeoJson.geometry.coordinates[0]
  const boundaryPath = districtBoundary.map(
    ([longitude, latitude]) => new kakao.maps.LatLng(latitude, longitude),
  )
  ...
  const label = document.createElement('div')
  label.className = 'district-map-label'
  label.textContent = district11GeoJson.properties.name
...
function displayDistrict12Boundary(kakao) {
  const districtBoundary = district12GeoJson.geometry.coordinates[0]
  const boundaryPath = districtBoundary.map(
    ([longitude, latitude]) => new kakao.maps.LatLng(latitude, longitude),
  )
  ...
  const label = document.createElement('div')
  label.className = 'district-map-label'
  label.textContent = district12GeoJson.properties.name
...
function displayDistrict13Boundary(kakao) {
  const districtBoundary = district13GeoJson.geometry.coordinates[0]
  const boundaryPath = districtBoundary.map(
    ([longitude, latitude]) => new kakao.maps.LatLng(latitude, longitude),
  )
  ...
  const label = document.createElement('div')
  label.className = 'district-map-label'
  label.textContent = district13GeoJson.properties.name
...
function displayDistrict14Boundary(kakao) {
  const districtBoundary = district14GeoJson.geometry.coordinates[0]
  const boundaryPath = districtBoundary.map(
    ([longitude, latitude]) => new kakao.maps.LatLng(latitude, longitude),
  )
  ...
  const label = document.createElement('div')
  label.className = 'district-map-label'
  label.textContent = district14GeoJson.properties.name
...
function displayDistrict16Boundary(kakao) {
  const districtBoundary = district16GeoJson.geometry.coordinates[0]
  const boundaryPath = districtBoundary.map(
    ([longitude, latitude]) => new kakao.maps.LatLng(latitude, longitude),
  )
  ...
  const label = document.createElement('div')
  label.className = 'district-map-label'
  label.textContent = district16GeoJson.properties.name
...
function createKakaoMap() {
  ...
  kakao.maps.load(() => {
    try {
      ...
      displayDistrict4Boundary(kakao)
      displayDistrict5Boundary(kakao)
      ...
      displayDistrict11Boundary(kakao)
      displayDistrict12Boundary(kakao)
      displayDistrict13Boundary(kakao)
      displayDistrict14Boundary(kakao)
      displayDistrict15Boundary(kakao)
      displayDistrict16Boundary(kakao)
