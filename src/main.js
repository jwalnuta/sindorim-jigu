            ...
            <aside class="boundary-tool" aria-labelledby="boundary-tool-heading">
              <div class="boundary-tool__heading">
                <div>
                  ...
                  <h3 id="boundary-tool-heading">3지구 경계 입력</h3>
                </div>
...
function getDistrictName(longitude, latitude) {
  ...
  const district1Boundary = district1GeoJson.geometry.coordinates[0]
  const district2Boundary = district2GeoJson.geometry.coordinates[0]
  const district3Boundary = district3GeoJson.geometry.coordinates[0]
  const isInDistrict1 = isPointInPolygon(point, district1Boundary)
  const isInDistrict2 = isPointInPolygon(point, district2Boundary)
  const isInDistrict3 = isPointInPolygon(point, district3Boundary)

  if (isInDistrict1 && isInDistrict2) {
    return '1지구와 2지구 경계가 겹치는 위치'
  }

  if (isInDistrict1 && isInDistrict3) {
    return '1지구와 3지구 경계가 겹치는 위치'
  }

  if (isInDistrict2 && isInDistrict3) {
    return '2지구와 3지구 경계가 겹치는 위치'
  }

  if (isInDistrict1) {
    return district1GeoJson.properties.name
  }

  if (isInDistrict2) {
    return district2GeoJson.properties.name
  }

  if (isInDistrict3) {
    return district3GeoJson.properties.name
  }

  return '아직 등록되지 않은 지구'
}
...
function setupBoundaryTool(kakao) {
  ...
  boundaryTool.startButton.addEventListener('click', () => {
    ...
    setBoundaryGuide('지도에서 3지구 경계의 꼭짓점을 차례대로 선택하세요.')
  })
  ...
  boundaryTool.resetButton.addEventListener('click', () => {
    ...
    boundaryTool.startButton.textContent = '3지구 경계 그리기'
    updateBoundaryPreview()
  ...
  boundaryTool.completeButton.addEventListener('click', () => {
    ...
    setBoundaryGuide('3지구 경계를 완성했습니다. 아래에서 GeoJSON 좌표를 확인하세요.')
  })
